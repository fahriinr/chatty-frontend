...
const registerSchema = z
  .object({
    fullName: z.string().trim().min(2, { error: "Full name is to short" }),
    username: z
      .string()
      .regex(/^[a-zA-Z0-9._]{1,30}$/, {
        error:
          "Username can only contain lowercase letters, number, dot and underscore.",
      })
      .min(2, { error: "Username is too short" })
      .max(12, { error: "Username is too long" })
      .transform((val) => val.toLocaleLowerCase()),
    email: z.email({ error: "Invalid email address" }),
    password: z
      .string()
      .min(2, { error: "Password must be at least 6 characters long" }),
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div>
          ...
          <div className="relative mb-2">
            ...
            <input
              {...register("fullName")}
              className="text-sm w-full pl-10 pr-3 py-3 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
        ...
        <button
          ...
          className="w-full bg-sky-500 hover:bg-sky-600 disabled:opacity-70 disabled:cursor-not-allowed font-medium py-3 px-4 rounded-lg transition duration-300 flex justify-center items-center"
        >
          {mutation.isPending ? (
